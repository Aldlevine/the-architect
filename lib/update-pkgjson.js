  ...
  for (let pkgJsonPath of pkgJsonPaths) {
    ...
    for (let pkgSrcPath of pkgSrcPaths) {
      ...
      for (let dep of deps) {
        ...
        if (dep.indexOf(orgName) == 0) {
          pkgJsonDepsUsed[dep] = true;
          if (!(dep in pkgJson.dependencies)) {
            console.log(`Adding local "${dep}" to ${pkgJsonPath}`);
          }
          else if (pkgJson.dependencies[dep] !== mainPkgJson.version) {
            console.log(`Updating local "${dep}" to version "${mainDeps[dep]}" in ${pkgJsonPath}`);
          }
          pkgJson.dependencies[dep] = mainPkgJson.version;
          continue;
        }
        if (!(dep in mainDeps)) {
